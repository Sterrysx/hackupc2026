...
### 1.1 Qué simulamos
Un gemelo digital de la **HP Metal Jet S100**, una impresora industrial de Binder Jetting metálico, operando **24/7** bajo una serie temporal **diaria** entre **2015-01-01 y 2025-12-31**, evaluada sobre un conjunto de **10-20 ciudades de Europa** con condiciones ambientales y carga productiva variables.
...
Tras la entrega, un optimizador barrerá miles de combinaciones de intervalos de mantenimiento $(\tau_{nom,C1}, \tau_{nom,C2}, ..., \tau_{nom,C6})$, simulará la serie diaria completa **2015-01-01 → 2025-12-31** para cada combinación y ciudad seleccionada, y devolverá el calendario que minimiza el coste total anual sujeto a disponibilidad > 95 %.
...
**Paso de integración recomendado:** $\Delta t = 1$ día.

> Implementación recomendada: cada tick representa un día calendario y se convierte internamente a **24 horas equivalentes de operación** para actualizar $\tau_{mant,i}$, $L_i$ y el desgaste acumulado. Como $\lambda_i$ está parametrizada a partir de vidas útiles en horas, en código conviene aplicar $H_i(t + 1\ \text{día}) = H_i(t) - \lambda_i(t) \cdot 24$.
...
|---|---|---|
| Ciudad | Categórica | Seleccionar 10–20 ciudades europeas |
| Temperatura ambiente diaria $T_{amb}$ | Serie temporal por fecha y ciudad | Fase 1: mock data; Fase 2: temperatura ambiental observada real |
| Humedad relativa diaria $H$ | Serie temporal por fecha y ciudad | Puede mockearse al inicio y refinarse después |
| Presión atmosférica diaria $P_{atm}$ | Serie temporal por fecha y ciudad | Presión del aire de ese día; mock al inicio y sustituible por dato observado real |
| Concentración polvo $c_p$ | Función de $H_{C1}$ | $c_p = c_{p,0} \cdot (2 - H_{C1})$ |
| Volumen de trabajo (jobs/día) | Configurable | Derivable desde un perfil diario equivalente |
...
BUCLE TEMPORAL: Δt = 1 día, total = 2015-01-01 → 2025-12-31 = 4.018 días

  PARA CADA TICK t:
    1. Leer/calcular variables externas dinámicas de la ciudad activa (T_amb, H, P_atm, c_p, jobs/día)
    ...
    PARA CADA componente i:
      ...
      8. H_i = H_i - λ_i · 24 h equivalentes
      ...
      13. τ_mant,i += 24 h equivalentes
      14. L_i += 24 h equivalentes
...
|---|---|---|
| **Coste total anual** | Suma de preventivos + correctivos / 11 | € / año |
| Coste preventivo anual | Solo mantenimientos programados | € / año |
| Coste correctivo anual | Solo averías | € / año |
| Nº fallos por componente | Eventos correctivos en 2015-2025 | nº |
| Nº mantenimientos por componente | Eventos preventivos en 2015-2025 | nº |
| Disponibilidad de máquina | $(96.432 - downtime\ total\ acumulado) / 96.432$ | % |
| Health Index medio por componente | Media temporal de $H_i$ | adim. |
| Tiempo en cada estado | Horas equivalentes en OK / WARNING / CRITICAL / FAILED | h |
...
│   └── environment.yaml     # Perfiles diarios por ciudad (mock primero, reales después)
├── core/
...
- **Reproducibilidad:** semilla aleatoria configurable; las series diarias por ciudad deben poder generarse de forma determinista en modo mock y sustituirse más adelante por observaciones reales sin tocar el core del simulador.
...
2. **Test de ingestión climática diaria:** para una ciudad y una fecha concreta, verificar que el simulador consume exactamente la temperatura ambiente y humedad asociadas a ese día.
3. **Test de bucle térmico:** con $H_{C5}$ y $H_{C6}$ ambos en 0,3, verificar que el cap de 2,5 sobre el producto se aplica y la simulación no diverge.
4. **Test de mantenimiento extremo:** $\tau_{nom,i} = 24$ horas equivalentes → coste preventivo enorme, coste correctivo cercano a cero. Comprueba el extremo del barrido.
5. **Test de no-mantenimiento:** $\tau_{nom,i} = \infty$ → todos los componentes acaban en correctivo, disponibilidad cae. Comprueba el otro extremo.
...
1. **Modelo:** 3 subsistemas × 2 componentes cada uno = 6 componentes con Health Index $\in [0,1]$ y degradación gobernada por la fórmula $\lambda_i = \lambda_{0,i} \cdot f_{ext,i} \cdot f_{int,i} \cdot f_{cross,i}$ (todos los factores valen 1 en nominal), integrada a resolución diaria.
2. **Realismo incremental:** datos verificados de la HP Metal Jet S100 (build volume, printheads, potencias, ambiente) con costes y vidas útiles estimadas como parámetros calibrables, sobre una base climática diaria por ciudad que puede arrancar con mock data y luego sustituirse por temperatura ambiental real.
