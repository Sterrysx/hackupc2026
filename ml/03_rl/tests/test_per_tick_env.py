...
def test_printer_stepper_matches_run_printer_when_no_agent_action() -> None:
    ...
    from ml.lib.env_runner import rollout_with_agent, run_with_tau
    from backend.simulator.generate import load_configs
    from backend.simulator.schema import COMPONENT_IDS

    if not Path("data/train/fleet_baseline.parquet").exists():
...
def test_agent_can_force_preventive() -> None:
    ...
    from ml.lib.env_runner import rollout_with_agent
    from backend.simulator.schema import COMPONENT_IDS
...
def test_per_tick_env_observation_shape(short_dates) -> None:
    from ml.lib.rl import MaintenancePerTickEnv
...
def test_bootstrap_ci_on_uniform_cost_includes_mean() -> None:
    from ml.lib.rl import bootstrap_fleet_ci
...
def test_spr_module_runs_one_update_and_lowers_loss() -> None:
    ...
    from ml.lib.rl import SharedSPRFeaturesExtractor, SPRModule
...
def test_extract_spr_tuples_skips_episode_boundaries() -> None:
    from ml.lib.rl import extract_spr_tuples_from_buffer
