...
# Stage 00 EDA
uv run jupyter lab ml/00_eda/eda_fleet_baseline.ipynb

# Stage 01 baseline (pure simulator + Optuna; CPU-only is fine)
uv run jupyter lab ml/01_baseline/search.ipynb

# Stage 02 SSL (CUDA recommended — uses HuggingFace PatchTST)
uv run jupyter lab ml/02_ssl/00_generate_policy_runs.ipynb
uv run jupyter lab ml/02_ssl/01_pretrain.ipynb
uv run jupyter lab ml/02_ssl/02_finetune_rul.ipynb
uv run jupyter lab ml/02_ssl/03_surrogate_search.ipynb

# Stage 03 RL+SSL (uses Stage 02 encoder; CPU is fine, GPU helps PPO updates)
uv run jupyter lab ml/03_rl/00_setup_and_sanity.ipynb
uv run jupyter lab ml/03_rl/01_train_ppo.ipynb
uv run jupyter lab ml/03_rl/02_eval_test.ipynb
uv run jupyter lab ml/03_rl/03_compare.ipynb
uv run jupyter lab ml/03_rl/04_per_tick_recurrent_ppo.ipynb

# Stage 04 results comparison
uv run jupyter lab ml/04_models/results/compare_01_02_03.ipynb

# Tests (lib code only; notebooks are not exercised in CI)
uv run pytest backend/simulator/tests ml/01_baseline/tests ml/02_ssl/tests ml/03_rl/tests
```
...
in `ml/lib/`; the numbered folders only host scripts, notebooks, and
artefacts. Notebooks import via `from ml.lib.* import …`, which is
