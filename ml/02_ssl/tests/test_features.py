...
from ml.lib.data import DEFAULT_FLEET_PATH
from ml.lib.features import (
    CALENDAR_COLS,
    COUNTER_COLS,
...
def test_project_root_resolves_independently_of_cwd(tmp_path, monkeypatch) -> None:
    ...
    from ml.lib.data import DEFAULT_FLEET_PATH as canonical
    ...
    expected_models_dir = PROJECT_ROOT / "ml" / "02_ssl" / "models"
    ...
    assert canonical.as_posix().endswith("data/train/fleet_baseline.parquet")
...
def test_stratified_printer_split_balances_climates() -> None:
    ...
    from ml.lib.data import stratified_printer_split
    from backend.simulator.generate import build_printer_city_map, load_configs
    from backend.simulator.schema import CLIMATE_CATEGORIES
