...
Today the SDG writes a single parquet at `data/train/fleet_baseline.parquet` covering 2015-01-01 → 2024-12-31 (3 653 days × 100 printers). Both ML training and the live React dashboard read from that one file. That conflates two different uses:
...
|---|---|---|---|
| `train` | 2015-01-01 → 2024-12-31 | Deterministic city-cosine (`backend/simulator/core/weather.py:get_drivers`) — calibrated from real climatology in `cities.yaml` | Stage 00 EDA, Stage 01 Optuna baseline, Stage 02 SSL pretrain + RUL head, Stage 03 PPO RL, Stage 04 results |
| `futureSD` | 2025-01-01 → 2034-12-31 | Cosine baseline **+ stochastic perturbation** seeded for byte-determinism | FastAPI `/twin/*` endpoints, React dashboard, backend.agent grounded retrieval |
...
`backend/simulator/core/weather.py:get_drivers(city, date)` is a deterministic seasonal cosine:
...
The `T_mean_annual` and `T_amplitude` per city in `backend/simulator/config/cities.yaml` are tuned against real multi-year averages from Open-Meteo. So while no specific date in 2015–2024 carries a recorded weather observation, the *climatology* is real. We treat this as the "real temperatures" baseline for the train set.
...
| File | Today | Change |
|---|---|---|
| `backend/simulator/generate.py:17` | `DEFAULT_OUTPUT_PATH = Path("data/train/fleet_baseline.parquet")` | Become a function that takes a `dataset: Literal["train", "futureSD"]` arg. Two output paths: `data/train/fleet_baseline.parquet`, `data/futureSD/fleet_baseline.parquet`. |
| `backend/simulator/generate.py:18-22` | hard-coded `START_DATE`, `END_DATE`, `EXPECTED_DAYS` | Pulled from a per-dataset config: `train` = 2015-01-01 → 2024-12-31; `futureSD` = 2025-01-01 → 2034-12-31. |
| `backend/simulator/generate.py:42-58` | one writer | Looped over both datasets, or a CLI flag `--dataset`. Recommended: `python -m sdg.generate --dataset train` and `--dataset futureSD`. |
| `backend/simulator/core/weather.py` | `get_drivers(city, date)` only | Add `get_future_drivers(city, date, rng)` and a thin dispatcher `get_drivers_for_dataset(dataset, city, date, rng)`. |
| `backend/simulator/core/simulator.py:246` | `weather_drivers = get_drivers(...)` | Threads `dataset` through `_simulate_one_day` so the right weather function fires. The Component / degradation code is untouched. |
| `backend/simulator/config/cities.yaml` | climatology only | Add `sigma_T_daily`, `sigma_H_daily` per city for the futureSD noise model. Default ~30 % of `T_amplitude`. |
| `backend/simulator/schema.py` | one schema | Schema is shared — both parquets use the same `FINAL_SCHEMA`. No schema change. |
| `backend/agent/twin_data.py:24` | `DEFAULT_PARQUET_PATH = "data/train/fleet_baseline.parquet"` | `DEFAULT_PARQUET_PATH = "data/futureSD/fleet_baseline.parquet"` — frontend now sees the future. |
| `app.py:353` | comment | Update doc-comment to reflect the futureSD source. |
| `ml/lib/data.py:13` | `DEFAULT_FLEET_PATH = .../"fleet_baseline.parquet"` | `DEFAULT_FLEET_PATH = .../"train"/"fleet_baseline.parquet"` — ML training stays on the historical set. |
| `ml/02_ssl/tests/test_features.py:88` | `endswith("data/train/fleet_baseline.parquet")` | Update the path assertion. |
| `ml/03_rl/tests/test_per_tick_env.py:22` | `Path("data/train/fleet_baseline.parquet")` | Same. |
| `tests/simulator/test_generation.py` | regenerates one parquet via `main(tmp_path)` | Parameterise over `["train", "futureSD"]` so both byte-deterministic. |
...
- `train` parquet keeps the existing `add_rul_labels` pass (`backend/simulator/labels.py`). Labels are looked up forward inside the train window — so failures near 2024-12-31 produce nullable RUL for the right-censored tail. ML training already handles this.
...
- `ml/lib/data.py:DEFAULT_FLEET_PATH` → `data/train/fleet_baseline.parquet`
...
- The trained encoder + RUL head + PPO policy are then published as artifacts under `ml/02_ssl/models/` and `ml/03_rl/models/`. The backend.agent's `forecast.py` loads them and applies them to the **futureSD** parquet at request time. This is the same dispatch path as today — only the source parquet changes.

There is **no train/test/val split refactor**. `ml/lib/data.py:TRAIN_PRINTERS / VAL_PRINTERS / TEST_PRINTERS` (printer 0..69 / 70..84 / 85..99) keeps applying *within* the train dataset. The futureSD set is held entirely separate; it is not used for hyperparameter search or model selection.
...
4. **Phase D — Repoint readers.** Switch `backend/agent/twin_data.py` to futureSD; switch `ml/lib/data.py` to train. Update the front-end start-date constant. Run the full test suite — expect green.
5. **Phase E — Notebook reruns.** `./train.sh 0..4` rebuilds Stages 00–04 against the train parquet only. Stage-02 retrain refreshes the encoder/head; the dashboard's analytic-fallback exit hatch (`backend/agent/forecast.py:active_path`) kicks back over to `"ssl"` automatically.
6. **Phase F — Drop the legacy single parquet.** Delete `data/train/fleet_baseline.parquet` (the path-without-prefix) once everything has moved. Add a one-line migration note to the project root README so a fresh clone knows where to look.
